...
class Handler(BaseHTTPRequestHandler):
    payload={}; jobs_payload={"total_count":1,"jobs":[{"id":101,"name":"Validate","html_url":"https://github.com/owner/repo/actions/runs/1/jobs/101","status":"completed","conclusion":"success"}]}; calls=0; headers={}; jobs_status=200; runs_status=200
    def do_GET(self):
...
class CICapabilityTests(unittest.TestCase):
    ...
    def setUp(self): Handler.calls=0; Handler.payload={'workflow_runs':[]}; Handler.jobs_payload={'total_count':1,'jobs':[{'id':101,'name':'Validate','html_url':'https://github.com/owner/repo/actions/runs/1/jobs/101','status':'completed','conclusion':'success'}]}; Handler.jobs_status=200; Handler.runs_status=200; os.environ.pop('GITHUB_TOKEN',None)
    ...
    def test_pending_wait_timeout(self): Handler.payload={'workflow_runs':[{'id':1,'head_sha':SHA,'status':'queued','conclusion':None,'name':'Validate','html_url':'https://github.com/owner/repo/actions/runs/1'}]}; p=self.run_tool(extra=('--wait','--timeout','1','--interval','1')); self.assertEqual(p.returncode,6)
