...
func New(stateDir string) Store {
	return Store{StateDir: stateDir, MaxBytes: DefaultMaxBytes, Retention: DefaultRetention}
}
...
func EventFor(ctxValues map[string]string, level, component, name string) Event {
	event := Event{Timestamp: time.Now().UTC(), Level: level, Component: component, Event: name}
