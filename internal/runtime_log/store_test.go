...
func testEvent(n string) Event {
	return Event{Timestamp: time.Date(2026, 8, 13, 0, 0, 0, 0, time.UTC), Level: "info", Component: "gateway", Event: n, OperationID: "op-1", Message: "safe"}
}

func TestStoreAppendReadFiltersAndRedacts(t *testing.T) {
	dir := t.TempDir()
	store := New(dir)
	if err := store.Append(Event{Timestamp: time.Now().UTC(), Level: "info", Component: "mcp", Event: "action_finish", Message: "authorization=hidden"}); err != nil {
		t.Fatal(err)
	}
	result, err := store.Read(Filter{Component: "mcp", Limit: 1})
...
func TestEventJSONIsBoundedAndStrict(t *testing.T) {
	...
	if err := (Event{Timestamp: time.Now().UTC(), Level: "info", Component: "gateway", Event: "bad\nline"}).Validate(); err == nil {
