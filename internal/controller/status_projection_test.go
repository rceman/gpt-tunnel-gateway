...
func TestProjectRuntimeStatus(t *testing.T) {
	base := Status{
		Gateway:      ProcessStatus{Running: true, IdentityValid: true},
		Tunnel:       ProcessStatus{Running: true, IdentityValid: true},
		GatewayReady: true,
		TunnelReady:  true,
		VersionMatch: true,
	}
	tests := []struct {
		...
		{name: "recovery failed", status: base, recovery: gatewayRecoveryState{Status: "degraded", Attempts: 3, Reason: "readiness timeout with secret=hidden"}, state: "degraded", degraded: true, mcp: true, reason: "readiness timeout with secret=[redacted]", recoveryAt: 3},
	}
...
func TestProjectRuntimeStatusBoundsRecoveryReason(t *testing.T) {
	status := projectRuntimeStatus(Status{}, gatewayRecoveryState{Status: "degraded", Reason: string(make([]byte, 1024))})
