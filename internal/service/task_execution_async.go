...
func (s *Service) TaskWorkAsync(ctx context.Context, in TaskWorkInput) (TaskWorkReceipt, error) {
	...
	operation, err := s.enqueueTypedDurableMutationWithIdentity(ctx, "task-work", in.ProjectID, in, taskWorkIdentity{ProjectID: in.ProjectID, TaskID: in.TaskID, HubRevision: s.localHubRevision(ctx)})
	if err != nil {
