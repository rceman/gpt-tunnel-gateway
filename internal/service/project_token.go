...
func (s *Service) ProjectToken(ctx context.Context, input ProjectTokenInput) (ProjectTokenResult, error) {
	...
	if root == "." || !filepath.IsAbs(root) {
		return ProjectTokenResult{}, fmt.Errorf("project token requires an absolute repository root")
	}
	if len(remotes) == 0 {
		return ProjectTokenResult{}, fmt.Errorf("project token requires a repository remote")
	}
	...
	actualRemote, err := s.Git.RemoteURL(ctx, project)
	if err != nil {
