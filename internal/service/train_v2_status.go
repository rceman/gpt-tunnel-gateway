...
func (s *Service) projectStatusTrainV2(ctx context.Context, id string, local config.ProjectConfig) (ProjectStatus, error) {
	...
	tasks, taskErr := s.TaskAuthoringList(componentCtx, TaskAuthoringListInput{ProjectID: id, Limit: MaxTaskListLimit})
	...
	if taskErr == nil && trainErr == nil {
		pure := trainv2.ProjectStatus(tasks.Tasks, trains)
