...
func (s *Service) attemptReportID(trainID, taskID string, attempt uint64) (string, error) {
	evidence, err := s.sharedTrainEvidence()
	if err != nil {
		return "", err
	}
	return evidence.AttemptReportID(trainID, taskID, attempt)
}
...
func (s *Service) commitSharedTrain(ctx context.Context, operationID string, train model.TrainV2, kind string) error {
	if s.Durability == nil {
		return fmt.Errorf("Shared durability is unavailable")
	}
	return trainv2.CommitSharedTrain(ctx, trainv2.StartDependencies{Shared: s.Durability, OperationID: operationID}, train, kind, train.UpdatedAt)
}
