...
type taskArchiveContract struct {
...
func (s *Service) TaskLifecycleArchive(ctx context.Context, projectID, taskID, actor, reason string) (model.TaskAuthoring, error) {
	...
	if !now.After(current.UpdatedAt) {
		now = current.UpdatedAt.UTC().Add(time.Nanosecond)
	}
	previous := current
	...
	contract, err := json.Marshal(taskArchiveContract{
		SchemaVersion:      1,
		TaskRevision:       current.Revision,
		TaskRevisionSHA256: current.RevisionSHA256,
	...
	opSum := sha256.Sum256(append([]byte(projectID+"\x00"+taskID+"\x00"), contract...))
	if err := s.Durability.CommitTaskArchive(ctx, sqlitestore.CommitTaskArchiveRequest{
		OperationID: "task-archive-" + hex.EncodeToString(opSum[:]),
		ProjectID:   projectID, TaskID: taskID, Revision: entity.Revision,
		PreviousTaskPayload: entity.Payload, TaskPayload: payload,
		FromStatus: previous.Status, Actor: actor, Reason: reason,
		Contract: contract, RecordedAt: now,
	}); err != nil {
		return model.TaskAuthoring{}, err
	}
	return current, nil
...
func (s *Service) TaskLifecycleListQuery(ctx context.Context, projectID, text, status string, typ model.TaskType, cursor string, includeArchived bool) (TaskLifecyclePage, error) {
	...
	page, err := s.Durability.QuerySharedLifecycle(ctx, sqlitestore.SharedLifecycleQuery{EntityType: "task", ProjectID: projectID, Text: text, Filters: filters, IncludeArchived: includeArchived || status == model.TaskAuthoringArchived, ExcludeSuperseded: true, ExcludeDone: status == "", Limit: sqlitestore.SharedLifecycleQueryMaxRows, Cursor: cursor})
	if err != nil {
		return TaskLifecyclePage{}, err
	}
