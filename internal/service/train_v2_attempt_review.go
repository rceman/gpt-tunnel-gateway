...
func (s *Service) TrainV2AttemptReview(ctx context.Context, in TrainV2AttemptReviewInput) (TrainV2AttemptReviewResult, error) {
	...
	review, err = loadOrReuseAttemptReview(evidence, review)
	if err != nil {
		return TrainV2AttemptReviewResult{}, err
	}
	reviewRef, err := evidence.WriteAttemptReview(review)
	if err != nil {
		return TrainV2AttemptReviewResult{}, err
	}
	if reviewRef != review.ID {
		return TrainV2AttemptReviewResult{}, fmt.Errorf("Attempt review persistence returned unexpected identity")
	}
	...
	if err := s.commitSharedTrain(ctx, durableMutationOperationID(ctx), train, "train-attempt-review", sqlitestore.SharedReplicaIntent{Kind: "attempt_review", EntityID: review.ID, ProjectID: in.ProjectID, Payload: reviewPayload}); err != nil {
		return TrainV2AttemptReviewResult{}, err
	}
