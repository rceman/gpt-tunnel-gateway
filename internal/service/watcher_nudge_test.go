...
func TestWatcherNudgeUsesExactTrainAttemptSession(t *testing.T) {
	...
	train, op, err := s.TrainV2Create(context.Background(), TrainV2CreateInput{ProjectID: "example", TaskIDs: []string{task.ID}, CreatedBy: "planner", WriteOptions: WriteOptions{ExpectedHubRevision: revision}})
	if err != nil {
		t.Fatal(err)
	}
	started, err := s.TrainV2Start(context.Background(), TrainV2StartInput{ProjectID: "example", TrainID: train.ID, StartedBy: "planner", WriteOptions: WriteOptions{ExpectedHubRevision: op.Hub.After}})
	if err != nil {
		t.Fatal(err)
	}
	receipt, err := s.WatcherNudge(context.Background(), WatcherNudgeInput{ProjectID: "example", Text: "continue from checkpoint"})
	if err != nil {
		t.Fatal(err)
	}
