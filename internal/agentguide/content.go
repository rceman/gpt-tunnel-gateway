...
func Canonical() Content {
	return Content{
		...
		Exploration:      "Start with targeted repo-local reads. If rg is available, use it as a convenience; otherwise use a bounded equivalent such as repo-local find, grep, or sed. Batch compatible checks, reuse one captured status/branch/HEAD result, and avoid repeated ls, find, status, or log scans. Keep all exploration inside the assigned repository/worktree; tool absence never broadens scope.",
		...
		ExecutionExample: "GOOD reposuite README-only proof: (1) run the compound assigned-worktree preflight; (2) rg --files -g 'README*' and inspect the bounded README with sed -n '1,200p'; (3) report the exact result. BAD: git log --all, git ls-remote, remote enumeration, ~/.local/share/gpt-tunnel-gateway scans, or long TODO archaeology before the supplied repo is understood.",
		CLIUsage:         "Zero-state: gpt-tunnel guide. Safe reads: gpt-tunnel project list; gpt-tunnel project read <project_id>; gpt-tunnel project status <project_id>; gpt-tunnel plan read <project_id>; gpt-tunnel plan render <project_id>; gpt-tunnel adr list <project_id>; gpt-tunnel adr read <project_id> <adr_id>. The CLI has no Task lifecycle read/list surface: use Planner-supplied Task state or the canonical Gateway action, never filesystem archaeology. gpt-tunnel task work/finalize are execution mutations; invoke only when Planner explicitly instructs. Do not use project update/register, ADR create, plan update, or agent register as an ordinary coding Agent.",
