...
func Canonical() Content {
	return Content{
		RoleAuthority:    "Planner owns project context and Task/ADR decisions. Lead owns long-running assigned-Task execution and review. Advisor is an optional stateless consultant. Worker is the persistent coding executor. Agent is a generic managed runtime, not a workflow role. Never create a lane, edit main, merge, rebase, reset, or cherry-pick.",
		...
		Testing:          "For ADR118 rev2, Worker checks are focused/affected deterministic tests plus cache-aware scripts/test-fast.py; committed changes use origin/main (or main), GPT_TEST_BASE/--base overrides it, and the lane never uses -count=1. Start with the cheapest layer and use deterministic fakes or mocks for bulk service behavior. task/test runs scripts/test-full.sh once on the exact candidate and reuses a successful receipt only when revision, reviews, base, head/tree, branch, and gate profile match. Performance, profile, and race are separate lanes. Preserve assertions, fail-closed behavior, and boundary coverage.",
		...
		Architecture:     "The project uses durable Planner, Lead, Advisor, and Worker role Sessions. Agent is a generic managed runtime and may back several roles only through distinct role-bound Sessions. Train and watcher state are not runtime or workflow-role authority.",
		Tail:             "agent/tail accepts {session?,lines?}. Omitted session selects the unique active role-bound Session; zero is an error and more than one requires an explicit canonical Session key such as HOM_GTW_W_a23df. The selected durable Session resolves internally to its stored Airelay ref.",
		StatusAwait:      "agent/status and agent/await supervise only managed Agent runtimes. For task/test, task/integrate, or other server mutations, use operation/read and bounded operation/await; never use agent/await or shell sleep for server-operation waiting. Runtime selection uses stored Agent bindings and role Sessions, not Session-key prefixes.",
		PromptInterrupt:  "agent/prompt and agent/interrupt accept an optional logical Agent selector; when omitted, they use the server-selected managed runtime. Prompt sends a bounded message; interrupt cancels the current turn and may submit a bounded replacement. Neither accepts a caller-selected Session identity.",
