...
func Canonical() Content {
	return Content{
		...
		CLIUsage:         "Zero-state: gpt-tunnel guide. Get host-local Planner token with gpt-tunnel session token <PROJECT_CODE>; MCP never mints it; list/read/guide/logs never echo it. Public session_start accepts only {token}; server resolves Gateway, project, role, and optional Agent. Reads: gpt-tunnel project list/read/status; plan read/render; adr list/read; before work, use gpt-tunnel task read <key>. Project discovery is project-scoped; Admin sessions are excluded. The CLI has no Task list surface: use Planner state or Gateway. task work/finalize are execution mutations. Worker submits through fixed CLI only. Ordinary coding Agents do not use project update/register, ADR create, plan update, or agent register.",
