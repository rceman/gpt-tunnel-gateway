...
func TestPromptWithProvenancePrefixesExactlyAtTransportBoundary(t *testing.T) {
	...
	if _, err := c.PromptWithProvenance(context.Background(), "project_master", "HOM_EXM_P_abcde", "[SD-ABCDEFGH] hello"); err != nil {
		t.Fatal(err)
	}
	...
	if got := string(data); got != "[HOM_EXM_P_abcde] [SD-ABCDEFGH] hello" {
...
func TestPromptUsesUTF8ByteBoundAndPreservesProvenanceMarker(t *testing.T) {
	...
	if _, err := c.PromptWithProvenance(context.Background(), "project_master", "HOM_EXM_P_abcde", ascii); err != nil {
	...
	if _, err := c.PromptWithProvenance(context.Background(), "project_master", "HOM_EXM_P_abcde", message); err != nil {
	...
	if got := string(data); got != "[HOM_EXM_P_abcde] "+message {
		t.Fatalf("marker or UTF-8 payload changed: %q", got)
	}
	var validation *MessageValidationError
	if _, err := c.PromptWithProvenance(context.Background(), "project_master", "HOM_EXM_P_abcde", message+"é"); !errors.As(err, &validation) || validation.Code != "CONTENT_TOO_LARGE" || validation.ActualBytes != MaxPromptBytes+2 {
...
func TestPromptPreservesEmptyAndNULValidationAsStructuredErrors(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			var validation *MessageValidationError
			_, err := c.PromptWithProvenance(context.Background(), "project_master", "HOM_EXM_P_abcde", test.text)
