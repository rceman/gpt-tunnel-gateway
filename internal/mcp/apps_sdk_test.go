...
func TestCanonicalSuccessfulOutputsMatchEveryDeclaredSchema(t *testing.T) {
	...
	samples := map[string]any{
		...
		"task_create": map[string]any{"task": task, "operation": operation}, "task_list": map[string]any{"tasks": []map[string]any{{"id": task.ID, "title": task.Title, "status": state.Status, "updated_at": now, "latest_run_id": nil, "latest_run_status": nil}}, "cursor": ""}, "task_next": map[string]any{"task": nil}, "task_read": publicPacket,
		...
		"git_log": map[string]any{"commits": []map[string]any{{"short_sha": strings.Repeat("a", 10), "commit": commit.Subject, "date": now}}, "cursor": ""}, "git_show": map[string]any{"text": "show"}, "git_tree": map[string]any{"paths": []string{"README.md"}},
