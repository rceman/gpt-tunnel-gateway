...
func taskLifecycleInputProperties() map[string]any {
	return map[string]any{"type": taskTypeSchema(), "scope": map[string]any{"type": "object", "additionalProperties": false, "properties": map[string]any{"files": outputArray(outputString()), "modules": outputArray(outputString())}}, "title": boundedADRString("Task title.", 3, 128), "summary": boundedADRString("Task summary.", 1, 256), "objective": boundedADRString("Task objective.", 3, 200000), "acceptance_criteria": outputArray(outputString()), "constraints": outputArray(outputString()), "priority": taskPrioritySchema(), "dependencies": outputArray(outputString()), "preparation_references": outputArray(outputString()), "metadata": map[string]any{"type": "object", "additionalProperties": outputString()}, "adr_relation": outputString(), "adr_references": outputArray(outputString()), "relation_type": outputEnum(model.RelationKindAuthority, model.RelationKindCorrects), "relation_target": boundedADRString("Canonical target key for the optional initial relation.", 1, 128)}
}
...
func taskLifecycleUpdateSchema() map[string]any {
	...
	delete(p, "relation_type")
	delete(p, "relation_target")
	p["key"] = outputString()
...
func taskLifecycleReadOutputSchema() map[string]any {
	p := taskLifecycleInputProperties()
	properties := map[string]any{"key": outputString(), "revision": outputInteger(), "title": p["title"], "summary": p["summary"], "status": outputEnum(model.TaskAuthoringPlanned, model.TaskAuthoringReady, model.TaskAuthoringDone, model.TaskAuthoringArchived), "type": p["type"], "scope": p["scope"], "objective": p["objective"], "acceptance_criteria": p["acceptance_criteria"], "constraints": p["constraints"], "priority": p["priority"], "dependencies": p["dependencies"], "preparation_references": p["preparation_references"], "metadata": p["metadata"], "adr_relation": p["adr_relation"], "adr_references": p["adr_references"], "relations": relationGroupedOutputSchema(), "created_at": outputDateTime(), "updated_at": outputDateTime()}
	return closedOutput(properties, "key", "revision", "title", "status", "type", "objective", "acceptance_criteria", "constraints", "dependencies", "preparation_references", "adr_relation", "adr_references", "relations", "created_at")
}
