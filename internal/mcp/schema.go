...
func reviewSnapshotOutputSchema() map[string]any {
	...
	artifact := closedOutput(map[string]any{"available": outputBoolean(), "error": outputString(), "status": outputString(), "summary": outputString(), "commits": outputArray(outputString()), "changed_files": outputArray(outputString()), "commands": outputArray(command), "deviations": outputArray(outputString()), "remaining_risks": outputArray(outputString()), "finished_at": timeField, "hub_commit": outputString(), "head": outputString(), "branch": outputString(), "worktree_clean": outputBoolean(), "notes": outputArray(outputString()), "recorded_at": timeField}, "available")
	...
	repo := closedOutput(map[string]any{"refresh_attempted": outputBoolean(), "refresh_succeeded": outputBoolean(), "refresh_error": outputString(), "default_branch": outputString(), "default_head": outputString(), "task_branch": outputString(), "task_branch_published": outputBoolean(), "task_branch_head": outputString(), "worktree": worktree, "evidence_head_reachable": outputBoolean(), "base_to_evidence": compare, "default_to_evidence": compare, "changed_files": outputArray(outputString()), "diff_stat": outputString()}, "refresh_attempted", "refresh_succeeded", "default_branch", "task_branch", "task_branch_published", "worktree", "evidence_head_reachable", "base_to_evidence", "default_to_evidence", "changed_files")
	...
	return closedOutput(map[string]any{"schema_version": outputInteger(), "run": run, "task": task, "report": artifact, "evidence": artifact, "repository": repo, "checks": outputArray(check), "review_state": outputString(), "next_action": outputString()}, "schema_version", "run", "task", "report", "evidence", "repository", "checks", "review_state", "next_action")
}
