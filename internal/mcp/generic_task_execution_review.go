...
const taskGuideCompletion = "A verified integration moves the Task to integrated and pending Planner acceptance, not done. Planner acceptance uses one concise same-project task-review Journal reference plus the applicable integration proof in task/complete; exact candidate, verification, integration and Task revision facts remain in their authoritative receipts. Completion and archive are status-only lifecycle events at the unchanged content revision; task/read(revision) remains immutable content while task/history includes lifecycle events."
