...
const taskGuideReview = "Lead performs technical review and rework at each stage: call task/review, then use its worktree and base with code/diff. Inspect only relevant files through targeted code/read, code/tree, or code/search calls; do not substitute broad worktree inventory. The submitted candidate carries production and tests together and is compared to the dispatch base. Rebase compares the accepted prior artifact to the rebased candidate; any changed artifact requires review and reverification. Lead may run authorized non-final staging, disposable E2E, or preflight, including focused post-Task integration checks after risky Tasks, subsets, or Track end; final project activate/release waits for source-bound Planner Track review."
const taskGuideVerification = "After accepted code, plus any required rebase review, Lead runs controlled asynchronous Task verification and integrates only verified evidence. Evidence goes through canonical journal/* actions; journal/contract is the sole stream-rules authority; there is no direct Lead-to-Planner channel, and owner/operator relay is only for semantic blockers or completed Track handoff, not execution proxy. ADR72 Gates 1-20, including the Gates 9/12/14/19/20 public-response evidence requirements, are the sole gate taxonomy. Canonical drift requires controlled rebase, renewed review where artifacts changed, and fresh verification."
const taskGuideCompletion = "A verified integration moves the Task to integrated and pending Planner acceptance, not done. Any multiple Workers are assigned at dispatch. Planner acceptance uses one concise same-project planner-notes journal reference plus the applicable integration proof in task/complete; exact candidate, verification, integration and Task revision facts remain in their authoritative receipts. Completion and archive are status-only lifecycle events at the unchanged content revision; task/read(revision) remains immutable content while task/history includes lifecycle events."
