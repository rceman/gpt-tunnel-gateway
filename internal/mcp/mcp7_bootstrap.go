...
func addMCP7BootstrapTools(add func(string, string, map[string]any, func(context.Context, json.RawMessage) (any, error)), s *Server) {
	add("session_start", "Create a durable Planner session from a host-local bootstrap token; Gateway, project, and role are resolved server-side.", nil, func(ctx context.Context, raw json.RawMessage) (any, error) {
	...
	add("guide", "Describe the bounded GPT Tunnel bootstrap sequence and server-authorized roles.", guidePublicInputSchema(), func(context.Context, json.RawMessage) (any, error) {
		return map[string]any{
			...
			"steps": []string{
				...
				"Retrieve the project Planner bootstrap token only through the authenticated gpt-tunnel project token operator command; MCP never mints or lists it.",
