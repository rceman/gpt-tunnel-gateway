...
func prepareLegacyTransportSession(t *testing.T, srv *Server, originalName string, body []byte) []byte {
	...
	projectCode := "EXM"
	record, err := mcpSQLiteSessionStore(t, srv.Service.Config.StateDir).Create(durableSession.CreateInput{ProjectID: projectID, ProjectCode: projectCode, Role: durableSession.RolePlanner, SessionType: durableSession.SessionTypeChatGPT})
	if err != nil {
