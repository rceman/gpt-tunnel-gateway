...
func TestCallbackActionsRejectDeliveryMutationAndRequireBoundSession(t *testing.T) {
	...
	started := genericStructured(t, sessionCall(t, server, map[string]any{"action": "start", "project_id": "example", "role": "worker", "session_type": "chatgpt", "session_ref": "runtime-worker"}))
	session := started["session"].(map[string]any)["session_id"].(string)
