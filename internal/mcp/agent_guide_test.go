...
func TestTSK594GuidesStateCurrentRoleDelegationAndEvidenceAuthority(t *testing.T) {
	...
	for _, concept := range []string{
		...
		"Friction, lesson, decision, and performance evidence goes through Track handoff, Journal, comments, or the standing evidence channels TSK609/619/632/633",
	...
	for _, concept := range []string{
		...
		"Lead never proxies a Worker submit or impersonates a Session",
		"Agents submit assigned-lane artifacts only through the fixed CLI gpt-tunnel task submit-code|submit-tests|submit-rebase, never native MCP",
