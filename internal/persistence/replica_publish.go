...
func (r hubReplica) PublishShared(ctx context.Context, intent PublishIntent) error {
	...
	case PublishTask:
		...
		return r.publishJSON(ctx, path, "gateway: publish Shared task "+task.ID, task, func(raw []byte) (bool, error) {
			...
			if latest.Revision == task.Revision {
				...
				if !equal {
					return false, fmt.Errorf("Hub task conflicts with Shared replica")
				}
				return true, nil
			}
			return false, nil
	...
	case PublishADR:
		...
		return r.publishJSON(ctx, path, "gateway: publish Shared ADR "+adr.ID, adr, func(raw []byte) (bool, error) {
			return immutableReplicaReplay(raw, adr, "Hub ADR")
		})
	...
	case PublishAgent:
		...
		return r.publishJSON(ctx, path, "gateway: publish Shared Agent "+agent.ProjectID+"/"+agent.AgentID, agent, func(raw []byte) (bool, error) {
			return versionedAgentReplay(raw, agent)
		})
	case PublishTrain:
		...
		return r.publishJSON(ctx, path, "gateway: publish Shared Train "+train.ID, train, func(raw []byte) (bool, error) {
			...
			if latest.Revision == train.Revision {
				...
				if !equal {
					return false, fmt.Errorf("Hub Train conflicts with Shared replica")
				}
				return true, nil
			}
			return false, nil
	...
	case PublishIntegrationReceipt:
		...
		return r.publishJSON(ctx, path, "gateway: publish Shared Train integration receipt "+receipt.TrainID, receipt, func(raw []byte) (bool, error) {
			...
			if latest.UpdatedAt.Equal(receipt.UpdatedAt) {
				equal, err := canonicalJSONEqual(raw, receipt)
				if err != nil {
					return false, err
				}
				if !equal {
					return false, fmt.Errorf("Hub integration receipt conflicts with Shared replica")
				}
				return true, nil
			}
			return false, nil
	...
	case PublishIntegrationOperation:
		...
		return r.publishJSON(ctx, path, "gateway: publish Shared Train integration operation "+operation.OperationID, operation, func(raw []byte) (bool, error) {
			...
			if latest.UpdatedAt.Equal(operation.UpdatedAt) {
				equal, err := canonicalJSONEqual(raw, operation)
				if err != nil {
					return false, err
				}
				if !equal {
					return false, fmt.Errorf("Hub integration operation conflicts with Shared replica")
				}
				return true, nil
			}
			return false, nil
	...
	case PublishWatcherGuide:
		...
		return r.publishJSON(ctx, path, "gateway: publish Shared watcher guide "+guide.ProjectID, guide, func(raw []byte) (bool, error) {
			...
			if latest.Revision == guide.Revision {
				...
				if !equal {
					return false, fmt.Errorf("Hub watcher guide conflicts with Shared replica")
				}
				return true, nil
			}
			return false, nil
	...
	case PublishAttemptReport:
		...
		return r.publishJSON(ctx, path, "gateway: publish Attempt report "+path, report, func(raw []byte) (bool, error) {
			return immutableReplicaReplay(raw, report, "Hub Attempt report")
		})
	case PublishAttemptReview:
		...
		return r.publishJSON(ctx, path, "gateway: publish Attempt review "+path, review, func(raw []byte) (bool, error) {
			return immutableReplicaReplay(raw, review, "Hub Attempt review")
		})
	default:
...
func immutableReplicaReplay[T any](raw []byte, expected T, kind string) (bool, error) {
	equal, err := canonicalJSONEqual(raw, expected)
	if err != nil {
		return false, err
	}
	if !equal {
		return false, fmt.Errorf("%s diverges from Shared replica", kind)
	}
	return true, nil
}

func versionedAgentReplay(raw []byte, expected model.Agent) (bool, error) {
	var latest model.Agent
	if err := decodeStrict(raw, &latest); err != nil {
		return false, err
	}
	if latest.UpdatedAt.After(expected.UpdatedAt) {
		return false, fmt.Errorf("Hub Agent is newer than Shared replica")
	}
	if latest.UpdatedAt.Equal(expected.UpdatedAt) {
		equal, err := canonicalValuesEqual(latest, expected)
		if err != nil {
			return false, err
		}
		if !equal {
			return false, fmt.Errorf("Hub Agent conflicts with Shared replica")
		}
		return true, nil
	}
	return false, nil
