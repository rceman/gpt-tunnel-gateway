...
func CommitSharedTrain(ctx context.Context, deps StartDependencies, train model.TrainV2, kind string, now time.Time) error {
	...
	mutation := sqlitestore.SharedMutation{OperationID: operationID, EntityType: "train", EntityID: train.ID, ExpectedRevision: int64(train.Revision - 1), Revision: int64(train.Revision), Kind: kind, Payload: payload, CreatedAt: now, Create: train.Revision == 1, ReplicaIntents: append([]sqlitestore.SharedReplicaIntent(nil), deps.ReplicaIntents...)}
	if kind == "train-v2-create" || kind == "train-v2-add" {
		mutation.TrainAdmissions = make([]sqlitestore.SharedTrainTaskAdmission, 0, len(train.Items))
		for _, item := range train.Items {
			mutation.TrainAdmissions = append(mutation.TrainAdmissions, sqlitestore.SharedTrainTaskAdmission{ProjectID: train.ProjectID, TaskID: item.TaskID, TrainID: train.ID})
		}
	}
	_, err = deps.Shared.CommitSharedMutation(ctx, mutation)
	return err
}
