...
func (r TaskRevision) StatusView() TaskRevisionStatus {
	return TaskRevisionStatus{
		SchemaVersion: r.SchemaVersion, ID: r.ID, TaskID: r.TaskID,
		TaskRevision: r.TaskRevision, RevisionSHA256: r.RevisionSHA256,
		ParentTaskRevision: r.ParentTaskRevision, Status: r.Status,
		Branch: r.Branch, BaseRevision: r.BaseRevision,
		SourceRunID: r.SourceRunID, SourceReportID: r.SourceReportID,
...
func TaskRevisionFromTask(task Task) TaskRevision {
	return TaskRevision{
		SchemaVersion: TaskRevisionSchemaVersion, ID: FormatTaskRevisionIDUnchecked(task.ID, 1),
		TaskID: task.ID, TaskRevision: 1, RevisionSHA256: task.SHA256,
		ProjectID: task.ProjectID, Title: task.Title, Objective: task.Objective,
		Branch: task.Branch, BaseRevision: task.BaseRevision,
		AcceptanceCriteria: append([]string{}, task.AcceptanceCriteria...),
		Constraints:        append([]string{}, task.Constraints...), RequiredGates: append([]string{}, task.RequiredGates...),
		WorkflowPolicyRevision: task.WorkflowPolicyRevision, OperationClass: task.OperationClass,
		EffectiveCIField: task.EffectiveCIField, EffectiveCIMode: task.EffectiveCIMode,
		WaitForCI: task.WaitForCI, CIBlocking: task.CIBlocking, AgentMayWait: task.AgentMayWait,
		Status: task.Status, CreatedBy: task.CreatedBy, CreatedAt: task.CreatedAt,
	}
}

func (r TaskRevision) Task() Task {
	return Task{
		SchemaVersion: SchemaVersion, ID: r.TaskID, SHA256: r.RevisionSHA256,
		ProjectID: r.ProjectID, Title: r.Title, Objective: r.Objective,
		Branch: r.Branch, BaseRevision: r.BaseRevision,
		AcceptanceCriteria: append([]string{}, r.AcceptanceCriteria...),
		Constraints:        append([]string{}, r.Constraints...), RequiredGates: append([]string{}, r.RequiredGates...),
		WorkflowPolicyRevision: r.WorkflowPolicyRevision, OperationClass: r.OperationClass,
		EffectiveCIField: r.EffectiveCIField, EffectiveCIMode: r.EffectiveCIMode,
		WaitForCI: r.WaitForCI, CIBlocking: r.CIBlocking, AgentMayWait: r.AgentMayWait,
		Status: r.Status, CreatedBy: r.CreatedBy, CreatedAt: r.CreatedAt,
	}
}
