...
func TestProjectCallbackValidation(t *testing.T) {
	valid := ProjectCallback{
		Callback: "notify",
		Event:    ProjectCallbackWorkFinishedEvent,
		URL: &ProjectCallbackURL{
			Method: "POST",
	...
	valid.Script = &ProjectCallbackScript{
		Path: "scripts/work-finished",
		Args: []string{"--bounded"},
	}
	if err := ValidateProjectCallback(valid); err != nil {
		t.Fatal(err)
	...
	for name, callback := range map[string]ProjectCallback{
		...
		"bad URL": {Callback: "notify", Event: ProjectCallbackWorkFinishedEvent, URL: &ProjectCallbackURL{
			Method: "POST",
			URL:    "file:///tmp/hook",
		}},
		"escaping script": {Callback: "notify", Event: ProjectCallbackWorkFinishedEvent, Script: &ProjectCallbackScript{
			Path: "../hook",
		}},
	} {
