...
func (d *Databases) ReconcileProjectBootstrap(ctx context.Context, in ProjectBootstrapUpdate) error {
	...
	if entity, err := d.ReadSharedEntity(ctx, "project_configuration", in.ProjectID); err == nil {
		if entity.Revision != int64(in.Configuration.Revision) || !bytes.Equal(entity.Payload, payload) {
			existing, decodeErr := DecodeCanonicalProjectConfigurationPayload(entity.Payload)
			if decodeErr != nil || !reflect.DeepEqual(existing, in.Configuration) {
	...
	if identifiers.found && identifiers.projectCode != in.ProjectCode {
		...
		statements = append(statements, upstream.Statement{SQL: `INSERT INTO shared_project_identifiers(project_id,project_code) VALUES(?,?)`, Args: []any{in.ProjectID, in.ProjectCode}, RequireRowsAffected: 1})
	}
	for _, sequence := range sequences {
	...
	for _, entityType := range []string{"task", "adr", "milestone"} {
		next := maxBootstrapNext(sequences, entityType, sequenceFallback(entityType, in.HubIdentifiers))
		statements = append(statements, upstream.Statement{SQL: `INSERT INTO shared_entity_sequences(entity_type,project_id,project_code,next_number) VALUES(?,?,?,?) ON CONFLICT(entity_type,project_id) DO UPDATE SET project_code=excluded.project_code,next_number=MAX(shared_entity_sequences.next_number,excluded.next_number)`, Args: []any{entityType, in.ProjectID, in.ProjectCode, next}, RequireRowsAffected: 1})
	}
...
func (d *Databases) ReadSharedProjectIdentifiers(ctx context.Context, projectID string) (model.ProjectIdentifiers, error) {
	...
	if !codeOK {
		return model.ProjectIdentifiers{}, fmt.Errorf("invalid Shared project identifier code")
	}
	taskCode, task, taskFound, err := d.ReadSharedSequence(ctx, "task", projectID)
	if err != nil {
		return model.ProjectIdentifiers{}, err
	}
	if !taskFound || taskCode != code {
		return model.ProjectIdentifiers{}, fmt.Errorf("Shared Task sequence is unavailable or conflicts with project identity")
	}
	adrCode, adr, adrFound, err := d.ReadSharedSequence(ctx, "adr", projectID)
	if err != nil {
		return model.ProjectIdentifiers{}, err
	}
	if !adrFound || adrCode != code {
		return model.ProjectIdentifiers{}, fmt.Errorf("Shared ADR sequence is unavailable or conflicts with project identity")
	}
...
func sequenceFallback(typ string, hubIDs model.ProjectIdentifiers) int64 {
	switch typ {
	case "milestone":
		return 1
	case "adr":
