...
func TestFreshBaselinesAreTheOnlyMarkersAndReopen(t *testing.T) {
	...
	assertFreshLocalMigrationMarkers(t, db.Local)
	...
	assertFreshLocalMigrationMarkers(t, reopened.Local)
}

func assertFreshLocalMigrationMarkers(t *testing.T, db *upstream.Store) {
	t.Helper()
	rows, err := db.Query(context.Background(), `SELECT version,name FROM schema_migrations ORDER BY version`)
	if err != nil {
		t.Fatal(err)
	}
	want := [][2]any{{localBaselineVersion, localBaselineName}, {localTokenUsageMigrationVersion, localTokenUsageMigrationName}, {localOperationMigrationVersion, localOperationMigrationName}, {localOperationAdmissionMigrationVersion, localOperationAdmissionMigrationName}, {localRelationMigrationVersion, localRelationMigrationName}, {localSessionBootstrapMigrationVersion, localSessionBootstrapMigrationName}, {localMessageMigrationVersion, localMessageMigrationName}, {localMessageCancellationMigrationVersion, localMessageCancellationMigrationName}, {localTaskExecutionMigrationVersion, localTaskExecutionMigrationName}}
	...
	for i, marker := range want {
		if rows.Rows[i][0] != marker[0] || rows.Rows[i][1] != marker[1] {
			t.Fatalf("fresh Local migration marker[%d]=%#v, want=%#v", i, rows.Rows[i], marker)
		}
	}
}
