...
var sharedLifecycleRegistry = map[string]sharedLifecycleDefinition{
	...
	"track": {
		...
		HistoryEntityColumn:   "entity_type",
		HistoryIDColumn:       "entity_id",
		SearchFields:          []string{"id", "milestone", "title", "summary", "status", "tasks"},
		FilterFields:          []string{"status", "milestone"},
		DefaultCreateStatus:   "planned",
		AllowedCreateStatuses: []string{"planned"},
		AllowedStatuses:       []string{"planned", "active", "ready", "review_pending", "accepted", "stale", "cancelled", "archived"},
		AllowedTransitions: map[string][]string{
			"planned":        {"active", "ready", "review_pending", "cancelled"},
			"active":         {"ready", "stale", "review_pending"},
			"ready":          {"active", "review_pending", "stale"},
			"review_pending": {"active", "ready", "accepted", "stale"},
			"accepted":       {"stale", "review_pending", "archived"},
			"stale":          {"active", "ready", "review_pending"},
			"cancelled":      {"archived"},
		},
		ArchiveStatus:       "archived",
		StatusMutationKind:  "status",
	...
	"rule": {
		...
		DefaultCreateStatus:   "proposed",
		AllowedCreateStatuses: []string{"proposed"},
		AllowedStatuses:       []string{"proposed", "accepted", "archived"},
		AllowedTransitions:    map[string][]string{"proposed": {"accepted", "archived"}, "accepted": {"archived"}},
		ArchiveStatus:         "archived",
		StatusMutationKind:    "status",
		ArchiveMutationKind:   "archive",
	},
