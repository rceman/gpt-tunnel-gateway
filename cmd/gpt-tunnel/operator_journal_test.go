...
func TestOperatorCLIRecordCheckpointAndHistoryRoutes(t *testing.T) {
	...
	if !strings.Contains(record, `"id": "EXM-JRN1"`) || !strings.Contains(record, `"status": "recorded"`) {
	...
	if !strings.Contains(checkpoint, `"id": "EXM-JRN2"`) || !strings.Contains(checkpoint, `"status": "checkpointed"`) {
	...
	if !strings.Contains(history, `"has_more": true`) || !strings.Contains(history, `"next_after_event_id": "EXM-JRN1"`) {
