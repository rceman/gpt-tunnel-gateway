...
func TestTSK654ProjectTokenLiveDaemonCwdResolutionAndDurability(t *testing.T) {
	...
	options := testutil.LiveCommandOptions{Dir: gateway.ProjectRoot, Env: map[string]string{"GPT_TUNNEL_ADMIN_SESSION": ""}}
	...
	isolated := gateway.MustCLI(t, testutil.LiveCommandOptions{Dir: t.TempDir(), Env: map[string]string{"GPT_TUNNEL_ADMIN_SESSION": ""}}, "project", "token", "ISO")
	isolatedToken := strings.TrimSpace(isolated.Stdout)
	if isolatedToken == "" || isolatedToken == firstToken {
		t.Fatal("explicit project code did not select an isolated token from outside a repository")
	}
	gateway.Restart(t)
	...
	unregistered := gateway.RunCLI(testutil.LiveCommandOptions{Dir: unregisteredRoot, Env: map[string]string{"GPT_TUNNEL_ADMIN_SESSION": ""}}, "project", "token")
	...
	nonRepo := gateway.RunCLI(testutil.LiveCommandOptions{Dir: t.TempDir(), Env: map[string]string{"GPT_TUNNEL_ADMIN_SESSION": ""}}, "project", "token")
	...
	legacy := gateway.RunCLI(testutil.LiveCommandOptions{Dir: gateway.ProjectRoot, Env: map[string]string{"GPT_TUNNEL_ADMIN_SESSION": ""}}, "session", "token", "AIR")
