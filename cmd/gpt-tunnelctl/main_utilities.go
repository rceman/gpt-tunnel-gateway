...
func usage() {
	fmt.Fprintln(os.Stderr, "usage: gpt-tunnelctl {install|init-config|upgrade [inspect|status]|doctor|diagnose-startup|state {check|repair --dry-run|repair --apply}|logs [gateway|tunnel|all] [lines]|version}")
	os.Exit(2)
}
